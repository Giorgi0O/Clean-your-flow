    ...
    <div className='h-screen'>
      ...
      <div className='relative top-[9%] lg:top-0 z-[100] w-screen h-5/6 lg:h-full overflow-y-scroll overflow-x-hidden'>
        ...
        <div id='second-section' className='mb-[100px] lg:mb-0 center flex-col lg:flex-row w-screen h-[130vh] lg:h-screen justify-between lg:p-16'>
          <Card iconNumber={1} color={'ciano'} titleId={'home.card1.title'} bodyId={'home.card1.body'} />
          <Card iconNumber={2} color={'verde'} titleId={'home.card2.title'} bodyId={'home.card2.body'} />
          <Card iconNumber={3} color={'rosa'} titleId={'home.card3.title'} bodyId={'home.card3.body'} />
        </div>
...
const Card = ({ iconNumber, titleId, bodyId, color }) => {

  return (
    <div className={`card-mirror ${color === 'rosa' ? 'card-mirror-rosa' : color === 'verde' ? 'card-mirror-verde' : 'card-mirror-ciano'}`}>
      <div className='h-1/2 w-full center m-8'>
        {iconNumber === 1 ? <ModalIcon1 /> : iconNumber === 2 ? <ModalIcon2 /> : <ModalIcon3 />}
      </div>
      <h1 className={`font-titolo font-bold text-3xl text-${color}-dark`}>
        <Trans i18nKey={titleId} >
          * <span className={`underline-wave decoration-${color}`}>*</span> *
        </Trans>
      </h1>
      <span className={`mt-2 font-body text-sm text-${color}`}>
        {
          iconNumber === 3 ?
